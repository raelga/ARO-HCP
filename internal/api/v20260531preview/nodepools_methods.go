...
func (h *NodePool) ConvertToInternal(existing *api.HCPOpenShiftClusterNodePool) (*api.HCPOpenShiftClusterNodePool, error) {
	...
	if h.Properties != nil {
		...
		// Conditions are read-only (server-managed) and are not accepted from client input.
		// They are preserved via CopyReadOnlyNodePoolValues.
	}
	...
	out := &NodePool{
		generated.NodePool{
			...
			Properties: &generated.NodePoolProperties{
				...
				Conditions:              newConditions(from.Properties.Conditions),
			},
