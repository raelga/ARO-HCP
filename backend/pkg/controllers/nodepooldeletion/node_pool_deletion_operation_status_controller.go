...
func (c *nodePoolDeletionOperationStatusController) SyncOnce(ctx context.Context, key controllerutils.HCPNodePoolKey) error {
	...
	// TODO we need to be aware that we are using operationcontrollers package
	// TODO we need to be aware that UpdateOperationStatus does not only update the operation status, but also
	// the resource (in this case here the nodepool) provisioning state, and also potentially unsetting ActiveOperationID
	// of the resource.
	// TODO we need to be aware that it also notifies the operation owner (if the operation's notificationuri)
	// TODO we need to be aware that it also creates a cosmos db transaction to include all db operations within it.
	// TODO UpdateOperationStatus first retrieves the resource from cosmos so we should not do a write of the
	// resource after this call because otherwise we would always fail as we retrieve it in the beginning of the controller
	// and we would have an outdated ETag if UpdateOperationStatus changes the resource and then the posterior write in our
