...
func (c *dispatchNodePoolCreate) SynchronizeOperation(ctx context.Context, key controllerutils.OperationKey) error {
	...
	csNodePoolHREF := ocm.GenerateAROHCPNodePoolHREF(clusterCSID.Path(), strings.ToLower(ext.Name))
	...
	nodePool.ServiceProviderProperties.ClusterServiceID = nodePoolCSInternalID
